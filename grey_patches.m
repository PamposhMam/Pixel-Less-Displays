...
nLevels = 32;  % or 64, depending on your use

% === 1. Choose a non-linear mapping to emphasize low intensities ===

% Option A: Square root (simple and effective)
mapped = sqrt(img);

% Option B: Logarithmic scaling (stronger low-end emphasis)
% mapped = log(1 + 9 * img) / log(10);  % scales to [0,1]

% Option C: Gamma correction with gamma < 1
% gamma = 0.5;
% mapped = img .^ gamma;

% === 2. Quantize into bins ===
quantized = floor(mapped * nLevels);
quantized = min(quantized, nLevels - 1);  % clip overflow
