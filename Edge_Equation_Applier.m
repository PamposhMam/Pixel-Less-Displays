...
filename = "LineEdge.bmp"; % Change this to the actual input file
...
for i = 1:num_rows
    for j = 1:num_cols
        ...
        if ~isempty(PixelEquations{i, j})
            ...
            % Debugging: Print stored equation
            disp(['Equation at (' num2str(i) ',' num2str(j) '): ' equation]);
            
            % Extract stored coordinate points
            points = sscanf(equation, '%f');  
            
            % Debugging: Print extracted points
            disp(['Extracted Points: ', num2str(points')]);
            ...
            % Debugging: Check if Holo_Func has nonzero values
            disp(['Max of Holo_Func at (' num2str(i) ',' num2str(j) '): ' num2str(max(abs(Holo_Func(:))))]);
            
            % Debugging: Display intermediate hologram
            figure; imshow(mat2gray(abs(Holo_Func))); title(['Holo_Func at (' num2str(i) ',' num2str(j) ')']);
...
cd(outputFolder); %Saves me filespace later on when i want to export to here
